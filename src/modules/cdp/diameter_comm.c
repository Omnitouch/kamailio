		...
		str* correlationID)
{
	...
	if (callback_f){
		if (is_req(message))
			cdp_add_trans(message,callback_f,callback_param,config->transaction_timeout,1,correlationID);
		else
...
AAAMessage* AAASendRecvMessage(AAAMessage *message, str* correlationID)
{
	...
	if (is_req(message)){
		sem_new(sem,0);
		t = cdp_add_trans(message,sendrecv_cb,(void*)sem,config->transaction_timeout,0,correlationID);
